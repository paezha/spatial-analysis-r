...
```{r message=FALSE}
#The function 'library' is used to load the data we want to work with. In this case, it is the isdas master package that we want to work with 
library(isdas)
```

The package includes a few datasets that will be used throughout the book:
```{r}
#The function `data` is used to check if a dataset is present within any loaded packages. In this case, we are looking for `snow_deaths` and `snow_pumps`
data("snow_deaths")
data("snow_pumps")
```
