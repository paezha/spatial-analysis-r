...
<!--chapter:end:01-Coverlet-Part-1.Rmd-->
...
```{r ch01-data-analysis-process, fig.cap= "\\label{fig:data-analysis-process} The process of doing data analysis (from Wickham and Grolemund, 2016)", echo=FALSE}
...
```{r ch01-RStudio-IDE, fig.cap= "\\label{fig:RStudio-IDE}The RStudio IDE", echo=FALSE}
...
```{r ch02-combination-of-operations}
2 * 3 + 5 < 2 * (3 + 5)
```
...
```{r ch02-examples-of-assignments}
...
```{r ch02-operations-with-indexing}
...
```{r ch02-indexing-by-variable-values}
...
```{r ch02-find-city-max-population}
...
```{r ch02-find-city-min-population}
...
```{r ch02-symbol-map-with-sized-symbols}
...
```{r ch02-prettifying-the-symbol-map}
ggplot(data = Cities, 
       aes(x = Longitude, y = Latitude)) + 
  geom_point(aes(size = Population)) + 
  geom_text(aes(label = Name), 
...
```{r ch02-visualization-example-1, fig.cap= "\\label{fig:visualization-example-1} Example of visualization: diagram of catchment areas for accessibility analysis (from [Paez, Higgins, and Vivona (2018)](https://doi.org/10.1371/journal.pone.0218773))", echo=FALSE}
knitr::include_graphics(rep("figures/02-Figure-1.png"))
```

```{r ch02-visualization-example-2, fig.cap= "\\label{fig:visualization-example-2} Example of visualization: accessibility to family doctors in Hamilton (from [Paez, Higgins, and Vivona (2018)](https://doi.org/10.1371/journal.pone.0218773))", echo=FALSE}
knitr::include_graphics(rep("figures/02-Figure-2.png"))
```

```{r ch02-visualization-example-3, fig.cap= "\\label{fig:visualization-example-3} Example of visualization: water sources (triangles) and households (circles) in a region in central Kenya (from [Paez et al. (2020)](https://doi.org/10.1016/j.jtrangeo.2019.102564))", echo=FALSE}
knitr::include_graphics(rep("figures/02-Figure-3.png"))
```

And, these are some figures created using `R` by talented people around the world.
```{r ch02-load-webshot2-package, include=FALSE}
if (!require("webshot2")) remotes::install_github('rstudio/webshot2')
library(webshot2) # Take Screenshots of Web Pages
```

```{r ch02-visualization-example-4-capture, include=FALSE, eval=FALSE}
webshot2::webshot("https://mastodon.online/@terence@fosstodon.org/111404498052039281", 
        file = "figures/02-Figure-4.png",
        selector = "div.detailed-status")
```

```{r ch02-visualization-example-4, fig.cap= "\\label{fig:visualization-example-4} Example of visualization: Income inequality in the US", echo=FALSE}

knitr::include_graphics("figures/02-Figure-4.png")
```

```{r ch02-visualization-example-5-capture, include=FALSE, eval=FALSE}
webshot("https://mastodon.online/deck/@terence@fosstodon.org/111652744513157011", 
        file = "figures/02-Figure-5.png",
        selector = "div.detailed-status")
```

```{r ch02-visualization-example-5, fig.cap= "\\label{fig:visualization-example-5} Example of visualization: Types of forests in Brazil", echo=FALSE}
knitr::include_graphics("figures/02-Figure-5.png")
```

```{r ch02-visualization-example-6-capture, include=FALSE, eval=FALSE}
webshot("https://mastodon.online/deck/@terence@fosstodon.org/111575222127539953", 
        file = "figures/02-Figure-6.png",
        selector = "div.detailed-status")
```

```{r ch02-visualization-example-6, fig.cap= "\\label{fig:visualization-example-6} Example of visualization: Earthquakes in California", echo=FALSE}
knitr::include_graphics("figures/02-Figure-6.png")
```

```{r ch02-visualization-example-7-capture, include=FALSE, eval=FALSE}
webshot("https://mastodon.online/deck/@terence@fosstodon.org/111450552351044906", 
        file = "figures/02-Figure-7.png",
        selector = "div.detailed-status")
```

```{r ch02-visualization-example-7, fig.cap= "\\label{fig:visualization-example-7} Example of visualization: Population change in Japan", echo=FALSE}
knitr::include_graphics("figures/02-Figure-7.png")
```
...
Load the libraries you will use in this activity:
```{r ch07-load-packages, message=FALSE}
library(tidyverse) # Easily Install and Load the 'Tidyverse'
```
...
```{r ch07-binomial-process-on-regular-coordinates}
...
```{r ch07-ch07-p-as-function-of-x-and-y}
...
```{r ch07-p-as-function-of-dist-to-centre}
...
Plot this pattern:
```{r ch07-plot-p-as-function-of-dist-to-centre}
ggplot() + 
  geom_point(data = subset(pattern1, 
                           events == 1), 
             aes(x = x, 
...
Load the libraries you will use in this activity:
```{r ch09-load-packages, message=FALSE, warning=FALSE}
library(isdas) # Companion Package for Book An Introduction to Spatial Data Analysis and Statistics
library(tidyverse) # Easily Install and Load the 'Tidyverse'
...
```{r ch09-change-size-of-quadrats}
...
knitr::include_graphics(rep("figures/17-Figure-1.jpg"))
```
...
knitr::include_graphics(rep("figures/17-Figure-2.jpg"))
```
...
As usual, it is recommended that you begin your work with a new session of `R`. At the very least, make sure that the working space is clear from extraneous items there may have been there from earlier. The command in `R` to clear the workspace is `rm` (for "remove"), followed by a list of items to be removed. To clear the workspace from _all_ objects, do the following:
```{r}
rm(list = ls())
```
...
```{r}
z_p.idw1 <- spatstat.explore::idw(Walker_Lake.ppp, power = 1)
```
...
z_p.idw05 <- spatstat.explore::idw(Walker_Lake.ppp, power = 0.5)
z_p.idw2 <- spatstat.explore::idw(Walker_Lake.ppp, power = 2)
z_p.idw5 <- spatstat.explore::idw(Walker_Lake.ppp, power = 5)
```
